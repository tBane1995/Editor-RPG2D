        ...
        if (this->object_and_texture_info != nullptr && this->object_and_texture_info->object!=nullptr) {
            ...
            hover_func = [this]() {
                if (tip == nullptr || (tip != nullptr && tip->btn != this)) {
                    ...
                    if (tip != nullptr)
                        delete tip;
                    tip = new Tip(ConvertUtf8ToWide(this->object_and_texture_info->object->name), pos, this);       // TO-DO delete convert and use std::wstring
                }
...
public:
    ...
    void setBuildingsToPalette() {
        ...
        buildings.push_back(new Building("assets/buildings/mud_house.building"));
        buildings.push_back(new Building("assets/buildings/stone_house.building"));
        buildings.push_back(new Building("assets/buildings/wooden_house.building"));
        buildings.push_back(new Building("assets/buildings/brick_house.building"));
        buildings.push_back(new Building("assets/buildings/mulch_house.building"));

        float part_size = 64;

        for (auto& building : buildings) {
            sf::Texture* texture = building->loadTextureWithDoors();
    ...
    void loadPalette() {
        for (int i = 0; i < paletteButtons.size(); i++) {
            ...
            int index;
            (scrollbar == nullptr) ? index = i + 0 * size.x : index = i + scrollbar->scroll_value * size.x;

            GameObjectAndTextureInfo* gameobject_and_texture_info;
            (index < gameObjectsAndTexturesInfo.size() && gameObjectsAndTexturesInfo[index]!=nullptr) ? gameobject_and_texture_info = gameObjectsAndTexturesInfo[index] : gameobject_and_texture_info = nullptr;
            ...
            if (object != nullptr) {
                ...
                paletteButtons[i]->hover_func = [this, button]() {
                    if (tip == nullptr || tip->btn != button) {
                        ...
                        if (tip != nullptr)
                            delete tip;
                        tip = new Tip(ConvertUtf8ToWide(button->object_and_texture_info->object->name), pos, button);       // TO-DO delete convert and use std::wstring
                    }
                    };
    ...
    void create_scrollbar() {
        ...
        scrollbar_position.y = -screenHeight / 2.0f + 30 + 2 * 52 + 2 * 32;
        ...
        scrollbar = new Scrollbar(scrollbar_size, scrollbar_position, 0, scrollbar_max_value, 0, 8, ScrollbarPartSize::_20);
    }
    ...
    void createToolsButtons() {
        ...
        sf::Vector2f button_size(32, 32);

        sf::Vector2f sp;    // start position
        sp.x = screenWidth / 2.0f - 10 * button_size.x + button_size.x/2 - 10;
        sp.y = -screenHeight / 2.0f + 30 + 2 * 52 + button_size.y/2 + 5;
    ...
    void createGroupButtons() {
        ...
        btnGroupBuildings->setTexture(getSingleTexture("GUI/menuButtons/menuButton-buildings"));
        ...
        btnGroupDirections->setTexture(getSingleTexture("GUI/menuButtons/menuButton-directions"));
        ...
        if (type == PaletteType::MapEditor) {
            ...
            groupButtons[8] = btnGroupBuildings;
            groupButtons[9] = btnGroupDirections;
        }
        ...
        for (auto i = 0; i < 10; i++) {
            ...
            position.x = screenWidth / 2.0f - (4 - i % 5) * button_size.x - button_size.x/2;
            position.y = -screenHeight / 2.0f + i / 5 * button_size.y + button_size.y/2 + 30;
    ...
    void handleEvent(sf::Event& event) {
        ...
        // TO-DO scrollbar->on_scroll_func
