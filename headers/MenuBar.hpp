...
public:
    ...
    // Building Editor
    ButtonWithText* newBuildingBtn;     // do usunięcia, ponieważ budynki są dodawane z poziomio mapy
    ButtonWithText* loadBuildingBtn;    // do usunięcia, poniważ budynki są wczytywane wraz z mapą i z poziomu mapy edytowane
    ButtonWithText* saveBuildingBtn;    // do usunięcia, ponieważ budynki są zapisywane wraz z mapą
    ButtonWithText* closeBEBtn;         // pozostaje, do wychodzenia z Building Editora
    ...
    void createButtons() {
        ...
        loadBuildingBtn->onclick_func = [this]() {
            clickedMenuButton->isOpen = false;
            clickedMenuButton = nullptr;
            dialogs.push_back(new FileDialog(DialogType::OpenFile, L"Load Building", ".bld"));
            };
