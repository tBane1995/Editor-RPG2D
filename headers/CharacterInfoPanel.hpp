...
public:
	...
	public:
		...
		public:
			...
			Stat(std::wstring name, std::wstring value) {
				stat_name = new TextArea(name);
				stat_value = new TextArea(value);
			}
		...
		public:
			...
			NumbSelector(std::wstring name, short& value, short step) : Stat(name, std::to_wstring(value)) {
				...
				stat_value->setRectSize(sf::Vector2f(80, stat_value->getLineHeight()));
				std::cout << stat_value->getLineHeight() << "\n";
		...
		Statistics(CharacterInfoPanel* parent) : CharacterInfoPage(parent) {
			...
			// new stats
			stats.push_back(new Stat(L"NAME", ConvertUtf8ToWide(parent->_character->name)));
			...
			for (short i = 0; i < stats.size(); i++) {
				...
				if (stats[i] != nullptr) {
					...
					sf::Vector2f pos;
					pos.y = start_pos_y + i * stats[0]->stat_name->getLineHeight() + m;
					...
					rect_pos.x = pos.x + cam->position.x - parent->rect.getSize().x/2.0f + 2*corner_size;
