...
public:
	...
	TextArea* textarea;
	ButtonFromDialog* btn_yes;
	ButtonFromDialog* btn_no;
	ConfirmValue value;
	...
	Confirm(std::wstring text) : Dialog(DialogType::Confirm, sf::Vector2f(256+32,128+32)) {
		...
		btn_yes = new ButtonFromDialog("tak");
		btn_no = new ButtonFromDialog("nie");
		float x1 = 128 - btn_yes->rect.getSize().x;
		float x2 = 128 - btn_no->rect.getSize().x;
		float margin_bottom = 16;

		btn_yes->setPosition(sf::Vector2f(position.x - x1, position.y + rect.getSize().y/2.0f - dialog_border_width - 8 - margin_bottom));
		btn_no->setPosition(sf::Vector2f(position.x + x2, position.y + rect.getSize().y/2.0f - dialog_border_width - 8 - margin_bottom));
